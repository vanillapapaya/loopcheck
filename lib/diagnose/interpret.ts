...
읽는 사람은 그 게임을 우리보다 잘 안다. 지시하지 말고 제안한다.
"~하세요", "~해야 합니다" 대신 "~이 필요해 보입니다", "~하는 편이 좋아 보입니다", "~으로 생각됩니다"를 쓴다.

- 요약, 개선안 제목과 본문 → 서술형 (~합니다, ~입니다)
...
- 줄표(—)로 부연하지 않는다. 문장을 끊거나 괄호와 쉼표를 쓴다
...
한 문단에 비중(분모: 전체)과 비율(분모: 해당 집단)이 같이 나오면 값이 비슷할 때 같은 지표로 읽힌다.
나란히 쓸 때는 각각의 분모를 한 마디로 붙인다.

- O: \`저사양의 D1 잔존 32.0%(분모: 저사양 설치 1,816명). 이 등급이 전체 설치에서 차지하는 비중은 30.3%\`
- X: \`저사양은 D1 32.0%, 전체의 30.3% 차지\`
...
JSON 하나만 출력한다.
{
  "summary": "리포트 맨 위 요약. 서술형. 정확히 3문장이며 각 문장이 한 줄씩 따로 표시된다. 문제의 위치, 크기와 근거, 주의할 충돌 순서로 쓴다",
  "findings": [
    {
      "title": "고칠 지점 한 문장. 서술형 제안조로 쓴다 (~이 필요해 보입니다, ~하는 편이 좋아 보입니다). 명령형(~하세요, ~해라)을 쓰지 않는다",
